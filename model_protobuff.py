...
model = None#load_model('word_pred_Model4.h5')
...
def get_prob(model, pad_encoded):
    ...
    sess.run(init)
    predictions = sess.run(out_tensor, {in_tensor: pad_encoded})
    print(predictions)
...
def main_itr():
    ...
    for i in range(seq_len):
        ...
        for itr in range(beam_width**exp0): 
            ...
            pad_encoded = gen_word(model, tokenizer, seq_len, output_text[count][0], num_gen_words=3)
